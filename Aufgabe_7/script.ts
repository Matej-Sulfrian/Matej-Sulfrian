...
function drumbeat() {
    var beatElements:string [] = ["sounds/kick.mp3", "sounds/hihat.mp3", "sounds/snare.mp3", "sounds/hihat.mp3", "sounds/hihat.mp3", "sounds/kick.mp3", "sounds/snare.mp3", "sounds/hihat.mp3"];
    var index:number = 0;
    var tempo = setInterval(dnbBeat, 173);
    function dnbBeat() {
        var MyMelody:HTMLAudioElement = new Audio(beatElements [index]);
        MyMelody.play();
        index += 1;
        if (index > 7)
            index = 0;
